        ...
        std::vector<std::string>& output)
{
    ...
    while (start < input.size()){
        p = input.find(spliter, start);
        if (p == std::string::npos){
            output.push_back(input.substr(start));
            start = input.size();
        }else if (p > start){
            output.push_back(input.substr(start, p - start));
            start = p + spliter.size();
        }else{
            start += spliter.size();
        }
    }
...
{
    std::vector<std::string> output;
    Gen::Str::split(input, spliter, output);
    return output;
}
...
//    boost::algorithm::trim_if(output, boost::is_any_of(" \t\r\n"));
//    return output;
//}
...
{
    return md5(text.c_str(), (int)text.size());
}
...
unsigned short crc16(const unsigned char * buffer, int length)
{
    ...
    for (int i = 0; i < length; ++i){
        crc = CRC16_TABLE[(crc >> 8) ^ buffer[i]] ^ (crc << 8);
    }
    return crc;
}
...
boost::json::object merge2(
        boost::json::object &dst,
        const boost::json::object &data)
{
    ...
    for (; it != data.end(); ++it){
        if (!dst.contains(it->key())){
            dst.insert(std::make_pair(it->key(), it->value()));
            changes.insert(std::make_pair(it->key(), it->value()));
            continue;
        }
        if (dst.at(it->key()) != it->value()){
            dst.at(it->key()) = it->value();
            changes.insert(std::make_pair(it->key(), it->value()));
            continue;
        }
    }
...
{
    std::string text;
    int rc = Gen::IO::readFile(filePath, text);
...
{
    return boost::filesystem::create_directories(boost::filesystem::path(filePath));
}
...
{
    boost::filesystem::rename(boost::filesystem::path(src), boost::filesystem::path(dest));
    return true;
}
...
int readFile(const std::string& filePath, char * buffer, int length)
{
    int rc = 0;
    FILE * file = nullptr;
    rc = fopen_s(&file, filePath.c_str(), "rb");
    if (rc){
        rc = (int)fread(buffer, 1, length, file);
        fclose(file);
    }
    return rc;
}
...
int writeFile(const std::string& filePath, const char * buffer, int length)
{
    int rc = 0;
    FILE * file = nullptr;
    rc = fopen_s(&file, filePath.c_str(), "wb");
    if (rc){
        rc = (int)fwrite(buffer, 1, length, file);
        fclose(file);
    }
    return rc;
}
...
{
    return writeFile(filePath, text.c_str(), (int)text.size());
}
